...
model = HierVAE(args.vocab).cuda()

# model.load_state_dict(torch.load('./ckpt/tmp/model.30'))
# torch.save(model.encoder.state_dict(), './ckpt/enc_p.pt')
...
for epoch in range(args.load_epoch + 1, args.epoch):
    ...
    for batch in dataset:
        ...
        meters = meters + np.array([float(kl_div), 
            float(loss.item()), 
            wacc.cpu() * 100, 
            iacc.cpu() * 100, 
            tacc.cpu() * 100, 
